...
def preprocessing(results):
    ...
    # 不要な列を削除
    df.drop(["タイム", "着差", "性齢","後3F", "馬体重（増減）","単勝オッズ","人気"], axis=1, inplace=True)
...
print(imp)
results_test = pd.read_csv("keiba_test7.csv")
...
data = pd.DataFrame(index=range(result_t.shape[0]), columns=_labels[7:])
